...
class BasicComment(models.Model):
    ...
    replyto = models.ForeignKey(User, null=True, blank=True, on_delete=models.PROTECT,
                                related_name='%(class)s_answers_made')
...
class NewsFeedArticlesList(models.Model):
    ...
    def generate_articles(self, days, limit):
        ...
        articles = BasicArticle.objects.filter(allowed=True,  pubdate__gte=minimum_date)[:limit].select_subclasses()
        articles = [article for article in articles if article.can_see_article()]  # bad
